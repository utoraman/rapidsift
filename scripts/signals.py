...
# ── Signal Suppression (data-driven, regenerated by update_suppressions.py) ──
...
def should_suppress(ticker, signal_type, sector_map=None):
    """
    Check if a signal should be suppressed based on historical performance.

    Args:
        ticker: stock ticker
        signal_type: e.g. 'macd', 'adjusted_drop'
        sector_map: dict of ticker -> sector ETF (optional, loaded if needed)

    Returns:
        str reason if suppressed, None if signal is allowed.
    """
    if ticker in SUPPRESSED_TICKERS:
        return f"ticker {ticker} below baseline"

    if sector_map is None:
        sector_map = load_sector_map()
    sector = sector_map.get(ticker, "SPY")

    if (signal_type, sector) in SUPPRESSED_COMBOS:
        return f"{signal_type}+{sector} combo below baseline"
